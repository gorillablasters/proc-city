    ...
    for proc in psutil.process_iter(
        ...
        try:
            ...
            process = Process(
                ...
                read_count=(
                    pinfo["io_counters"].read_count if pinfo.get("io_counters") else 0
                ),
                write_count=(
                    pinfo["io_counters"].write_count if pinfo.get("io_counters") else 0
                ),
                read_bytes=(
                    pinfo["io_counters"].read_bytes if pinfo.get("io_counters") else 0
                ),
                write_bytes=(
                    pinfo["io_counters"].write_bytes if pinfo.get("io_counters") else 0
                ),
                nice=pinfo.get("nice"),
                connections=(
                    len(pinfo["net_connections"]) if pinfo.get("net_connections") else 0
                ),
            )
